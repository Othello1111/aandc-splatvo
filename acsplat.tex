...
MUST BE FILLED STIl ABSTRACT NEEDED !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
\citep[][\ascl{1110.012}]{1982QJRAS..23..485D} and a lot of IDL
...
Czech Republic), who contributes in defining the user requierments and
suggesting modifications of scientific funcions (with emphasis on optical
...
data). SPLAT-VO also supports double sideband spectra for sub-millimeter
...
well example scripts for controling the SPLAT-VO from command line.
...
\item Identification of spectral lines using simple supplied ASCII linelists and
...
\caption{Zeta Ophiuchi in $H_\alpha$ region from Ond\v{r}ejov Coud\`e spectrograph 700mm camera (green) and HEROS (blue) with overplotted line identification from custom line list of telluric lines. Note the deepr telluric lines on HEROS due to its almost double  spectral resolving power}
...
\cite{ssap} uses  the various physical parameters like $T_{\rm eff}$, $\log g$,
metalicity etc. for query instead of position,wavelength reqions and time
...
\item	Improvement of the Unit Conversion functions in SPLAT-VO;
\item Retrieve spectral line identifiers using SLAP \citep[Simple Line
  Access Protocol;][]{slap}. Currently SPLAT-VO uses simple formatted ASCII files for the decription of custom line list and some built-in list. The SLAP access will allow for the imediate retrieval of line lists from  big world-wide atomic and molecular  database like e.g. NIST or all databases from VAMDC consortium  (REFERECCES !!!!!!!)
\end{itemize}
