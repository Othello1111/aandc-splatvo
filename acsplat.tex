...
\item Continuum estimation using flexible curves similar to those expected to
  be drawn by a human. The \textsc{intep} procedure based on Hermite polynomials
...
The SSAP interface in SPLAT-VO is clearly the most important tool for working
...
additions to SPLAT-VO now offer control of all the optional parameters
...
\caption{TSAP Query for Vega-like Kurucz models. Note the additional parameters.}
...
As the VO evolves, adding new protocols and data models, SPLAT-VO needs to
...
SSAP, the new \texttt{DataLink} protocol \citep{datalink} was used. SPLAT-VO
is one of the first client implementations of \texttt{DataLink}, in this way
...
spectrum and echelle spectra (like those from UVES or HARPS) may have
...
of complete spectra in SPLAT-VO on a common computer, was a very inconvenient and
...
of a VO-based post-processing server, based on the \textsc{pleinpot} suite
